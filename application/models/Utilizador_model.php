...
class Utilizador_model extends CI_Model
{
        ...
        $query = $this->db->query("
              SELECT `employer_mode`.`id`,
                ...
                `employer_mode`.`adress_mode_id`,
                 IFNULL(IF(`employer_mode`.`num_bi` = '',NULL,`employer_mode`.`num_bi`),'<span style=\"color: #aaaaaa\">Não foi inserido o nº BI / Pass</span>') AS `num_bi`, 
              ...
              WHERE `employer_mode`.`tipo`>'0' AND `employer_mode`.`name` != 'Programador Munzambi Miguel' ORDER BY id DESC
        ");
    ...
    {
        if ($this->input->post('emp_description') != '' && $this->input->post('emp_city') != -1) {
            $dataAdress = array(
        ...
        } else if ($this->input->post('emp_description') != '' && $this->input->post('emp_city') != -1) {
            $data = array(
                'name' => $this->input->post('emp_name'),
                'Genero' => $this->input->post('emp_genero'),
                'tipo' => $this->input->post('emp_tipe'),
    ...
    public function update()
    {
        ...
        if ($this->input->post('adress_id') != '') {
            ...
            $this->db->trans_complete();
        } else if ($this->input->post('emp_description') != '') {
        ...
        //
        if ($this->input->post('emp_description') != '') {
            $data = array(
                'name' => $this->input->post('emp_name'),
                'Genero' => $this->input->post('emp_genero'),
                'tipo' => $this->input->post('emp_tipe'),
        ...
        //
        if ($this->input->post('emp_password') != '') {
            $dataconta = array(
                'user_name' => $this->input->post('emp_username'),
                'password' => $this->encryption->encrypt($this->input->post('emp_password')),
            ...
            } else if ($this->input->post('emp_username') != ''
                && $this->input->post('emp_password') != '') {
                $this->db->insert('section_mode', $dataconta);
            }
        }
    ...
    public function insertDefault()
    {
        ...
        $data = array(
            'name' => 'Programador Munzambi Miguel',
            'Genero' => 'Masculino',
            'tipo' => '2',
            'num_bi' => 'JDBC - 00THS - ASD - 208'
        );
        ...
        $dataconta = array(
            'user_name' => 'Root',
            'password' => $this->encryption->encrypt('993416970'),
            'employer_mode_id' => $this->employer_id
        );
        $this->db->insert('section_mode', $dataconta);
